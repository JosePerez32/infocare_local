...
const Memory = ({ onDataUpdate }) => {
  ...
  const [gaugeOrder, setGaugeOrder] = useState(["Commited","SWAP"]); // State for the gauges order
  const [alertVisible, setAlertVisible] = useState(false); // State to show the alert

  const texts = ["Commited","SWAP"];
  ...
  return (
    <Box m="20px">
      <Header title={`Memory for ${databaseName}`} subtitle="" />
