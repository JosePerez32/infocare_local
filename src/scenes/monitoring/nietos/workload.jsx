...
const PerformanceWorkload = ({ onDataUpdate }) => {
  ...
  const [gaugeOrder, setGaugeOrder] = useState(["Change/SEC","Select SEC","Returned SEC","Cash/SEC"]); // State for the gauges order
  const [alertVisible, setAlertVisible] = useState(false); // State to show the alert

  const texts = ["Change/SEC","Select SEC","Returned SEC","Cash/SEC"];
  ...
  return (
    <Box m="20px">
      <Header title={`Workload Performance ${databaseName}`} subtitle="" />
...
export default PerformanceWorkload;