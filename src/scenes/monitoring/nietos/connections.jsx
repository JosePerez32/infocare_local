...
const Connections = ({ onDataUpdate }) => {
  ...
  const [gaugeOrder, setGaugeOrder] = useState(["Active","Idle","Oldest"]); // State for the gauges order
  const [alertVisible, setAlertVisible] = useState(false); // State to show the alert

  const texts = ["Active","Idle","Oldest"];
  ...
  return (
    <Box m="20px">
      <Header title={`Connections for ${databaseName}`} subtitle="" />
