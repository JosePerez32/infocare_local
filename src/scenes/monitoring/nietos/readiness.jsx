...
const Readiness = ({ onDataUpdate }) => {
  ...
  const [gaugeOrder, setGaugeOrder] = useState(["WL Elapsed","WL Time","WL Locks","WL Sorts"]); // State for the gauges order
  const [alertVisible, setAlertVisible] = useState(false); // State to show the alert

  const texts = ["WL Elapsed","WL Time","WL Locks","WL Sorts"];
  ...
  return (
    <Box m="20px">
      <Header title={`Readiness for ${databaseName}`} subtitle="" />
